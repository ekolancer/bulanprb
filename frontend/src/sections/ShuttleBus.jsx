...
const RouteCard = ({ route, index }) => (
  ...
  >
    <Card className="p-5 sm:p-6 border border-gray-100/60 flex flex-col gap-4 h-full">
      {/* Header */}
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <span className="flex items-center gap-1 text-xxs text-text-secondary font-medium mt-0.5">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="flex flex-wrap gap-2">
          {route.times.map((t) => (
            <span
              key={t}
              className="px-3 py-1.5 rounded-xl bg-primary/5 text-primary text-xs font-extrabold tabular-nums"
            >
              {t}
            </span>
          ))}
      ...
      {/* Note */}
      {route.note && (
        <div className="flex items-start gap-2 text-xxs text-text-secondary border-t border-gray-50 pt-3">
...
export const ShuttleBus = () => {
  const [activeDay, setActiveDay] = useState(shuttleDays[1]); // Default ke D0

  const dayData = shuttleSchedule[activeDay];

  return (
    <section id="shuttle" className="py-16 sm:py-20 lg:py-24 bg-background">
      <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <span className="text-xs font-bold text-primary uppercase tracking-widest">Transportasi Gratis</span>
        ...
        {/* Day tabs */}
        <div className="flex items-center gap-3 overflow-x-auto pb-1 mb-8 sm:justify-center
          ...
          {shuttleDays.map((day) => {
            ...
            return (
              ...
              >
                <span>{d.label}</span>
                <span className={`text-xxs font-medium ${activeDay === day ? 'text-white/80' : 'text-text-secondary'}`}>
                  {d.date}
                </span>
              </button>
            );
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <p className="text-center text-xs text-text-secondary font-medium mb-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          {/* Notes */}
          <div className="lg:col-span-7 bg-white rounded-3xl shadow-soft border border-gray-100/50 p-6 sm:p-8">
            <h3 className="text-base sm:text-lg font-extrabold text-text-primary mb-5">
              Catatan Penting Penjemputan
            </h3>
            <ul className="flex flex-col gap-4">
              {shuttleNotes.map((note, i) => (
                <li key={i} className="flex gap-3 items-start">
                  <span className="w-6 h-6 rounded-full bg-primary/5 text-primary text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">
                    {i + 1}
                  </span>
          ...
          {/* Maps CTA */}
          <div className="lg:col-span-5 bg-primary rounded-3xl p-6 sm:p-8 flex flex-col gap-4 justify-between text-white">
            <div className="flex flex-col gap-3">
              <div className="w-12 h-12 rounded-2xl bg-white/10 flex items-center justify-center">
                <MapPin className="w-6 h-6" />
              </div>
              <h3 className="text-lg font-extrabold leading-tight">Lihat Peta Titik Penjemputan</h3>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-2 bg-white text-primary px-5 py-3 rounded-xl font-bold text-sm hover:bg-white/90 transition-colors w-fit"
            >
              Buka Google Maps
